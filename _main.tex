...
That said, it is time to turn our attention towards the outcome of our random experiment. Suppose we are randomly drawing a single item from our sample space \(S = \{A, B, C, D, E\}\). The \emph{outcome} of our experiment is a single item from our sample space, and we will talk about outcomes in terms of \emph{events}. Events are described by a \emph{subset} of distinct elements from the sample space. That is, they may be one or more outcomes combined. For example, we might say the event is \(C = \{A, B, C\}\), and if our outcome lands in the that subset (as either A, B, or C), then we shall say the event ``occurred''. For the sake of brevity, and positivity, I will refer to the occurrence of an event as a ``success'' (congratulations all around!). You can indicate a set is a subset by:
...
\[ \{A\}\:\cup\:\{B\} = \{A,B\} \]
Our random experiment would be successful if the outcome was A OR B.
\end{example}
...
\pandocbounded{\includegraphics[keepaspectratio]{_main_files/figure-latex/unnamed-chunk-8-1.pdf}}
...
\begin{example}
Suppose the average number of cars passing through an intersection between 5pm and 6pm is 10. What is the probability of exactly 10 cars passing through the intersection? What is the probability of \emph{at least one} car passing through the intersection? And, finally, what is the mean and variance of the random variable representing car count? We shall deal with each of these in turn.
...
In the last chapter, we learned about discrete random variables, expectations, variance, and some common distributions. In this chapter we will explore similar concepts for random variables with continuous spaces. We also get to learn about one of the most useful, prolific, and oddly named distributions: The normal distribution.
...
A continuous random variable is similar to the discrete random variables we discussed in the last chapter. However, the way we define the set of outcomes and the way we attribute probability to those outcomes is different. Continuous random variables are defined first and foremost by their \emph{space} - which is not countable or finite in the way the discrete spaces are (more on this in a moment). For example, we may define the set of outcomes for a continuous random variable to be all the values between and including 0 and 1, e.g., \(0 \leq X \leq 1\). Indeed, defining the space is the easy part. The tricky part is describing the randomness of a continuous variable in the form of a probability distribution.

Let's explore this endeavor a bit deeper with the range - \(0 \leq X \leq 1\). Suppose we would like to define a probability distribution for this random variable. One (un)reasonable thing to try would be to proceed as we would for a discrete random variable. We take a probability mass of 1 and spread it around over of the values in the range. That is to say, we could try to define a probability mass function for our continuous random variable. Take a moment to glance back and the properties of a discrete random variable (if you don't remember them). Specifically, over the \(n\) outcomes in the space of \(X\), a pmf must have

\[
\sum_{i = 1}^{n} p(x_i) = 1
\]

As it turns out, this property will be quite difficult to attain - because the values in a continuous range aren't countable. That is, we can't list them out in an orderly fashion (e.g.~like we could for the Bernoulli, binomial, and poisson distributions). Let's dig into this a little further to make it clear. Turning our attention back to our random variable with space \(0 \leq X \leq 1\), let's try to count all the values between 0 and 1. Suppose we've listed (or believe we have listed) all the values in the range, e.g.,
...
But to our surprise (or maybe to our non-surprise) there will \emph{always} be a number we missed during our enumeration. Note the underlined numbers in our list below.
