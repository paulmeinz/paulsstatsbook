...
\end{cases}
\]
As we have seen from the previous two sections, it helps to have quick ways of calculating expectations and variances. For each of the common distributions, we shall therefore present or derive formuli for each. As you know, in order to get the variance, we first need to expectation. With regards to a Bernoulli random variable (\(X\)) with some parameter \(p\), the expectation is

\[
E(X) = \sum_{x} xp(x) = 1(p) + 0(1-p) = p
\]
The expectation of a Bernoulli random variable is the parameter \(p\) - a.k.a. the probability of a success. Now that we have the expectation, we can calculate the variance. This is a smidge more complicated. I'll try my best to breakdown the steps. Recall the definition of the Bernoulli distribution:
\[
p(x) = 
\begin{cases}
p & x = 1 \\
...
\end{align}
\]
Now, we'll stop here and point something out. This equation has two terms - one before the + and one after. Each of these terms has a common factor of \(p(1-p)\). We're going to use the magic of the distributive property. Watch the underlines below.
...
\end{align}
\]
The variance of a Bernoulli random variable is therefore \(p(1-p)\).
