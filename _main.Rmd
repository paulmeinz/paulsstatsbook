...
When we are in the process of conducting quantitative research (and statistics), we measure things in numbers. You may have noticed that our previous discussions of discrete sample spaces used non-numeric descriptions - e.g., $S = \{H, T\}$ in the case of a coin flip. To make these sample spaces easier to work with, we shall in practice map the outcomes in a discrete sample space to numbers. For example, if we were interested in a coin flip, we wouldn't work directly with the values of $H$ and $T$. We would map the each outcome to a value, e.g., heads to the value of 1 and tails to the value of 0. As you will see in the coming sections, this will permit us to look at the mathematical properties of our random experiment and sample space.
...
1. $E(4Z) = 4E(Z) = 4(7) = 28$ - by property 2

2. $E(Z + 10) = E(Z) + 10 = 7 + 10 = 17$ - by property 3 ($a = 1$)

3. $E(1) = 1$ - by property 1

4. $E(4Z + 10) = E(4Z) + 10 = 4E(Z) + 10 = 28 + 10 = 38$ - by property 3
:::

Do you see how that made things a bit easier? We didn't have to go through the computation of the expectation of our function of $Z$. I bet you can tell by now I like to avoid long computations! Now we shall turn our attention towards property 4.
