...
You're in for a treat! We shall next discuss the pmf of this random variable (and others like it). Fair warning: This is one of my favorite pmfs of all time.
:::
...
::: {.example #binom}
Suppose we have three independent Bernoulli random variables - $X_1$, $X_2$, and $X_3$ - with with $p = 1/3$. We would like to know the probability that they sum to the value of 1. In order to get a value of 1 from our sum, we will need exactly 1 success and 2 non-successes at the resolution of our random process for each variable. There are three such combinations of outcomes that will yield this result. I've listed each in a set below. Note that the digit order in each outcome corresponds to variables 1, 2, and 3, respectively.

$$
\{1\,0\,0, 0\,1\,0, 0\,0\,1\}
$$
Each of these outcomes in our three-variable sample space would result in a value of 1. For example, with regards to the first element, we have $1 + 0 + 0 = 1$. The event $Y = \{1\,0\,0, 0\,1\,0, 0\,0\,1\}$ therefore fully specifies the scenario where our Bernoulli random variables sum to 1. Now we need to calculate the probability of this event, and for this, we will need the probability of each of the outcomes in the set $Y$. We know that our random variables are independent. We can multiply $P(X_1 = x_1)$, $P(X_2 = x_2)$, and $P(X_3 = x_3)$ to determine the probability of each of the outcomes in $Y$. 
...
$$
Each of the outcomes above has the same number of successes and the same number of non-successes. Therefore each has the exact same probability, and by the law of addition, $P(Y) = 4/27 + 4/27 + 4/27 = 12/27 = 4/9$.
:::
...
$$
\frac{1}{8} + \frac{1}{8} + \frac{1}{8} = 3\times\frac{1}{8}
$$
...
Now that we know how to calculate the number of combinations necessary for $z$ successes, we shall turn our attention to the probability of a single combination. Looking back at example \@ref(exm:binom), a single outcome had one success and two non-successes. The probability of a single outcome was therefore

$$
p(1-p)(1-p) = p(1-p)^2 = 1/3(1-1/3)^2 = 4/27
$$
