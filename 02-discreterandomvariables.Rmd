...
:::
Hopefully that example helped unpack the idea of the expectation. In this regard, we shall need a few more tools to proceed down the wonderful rabbit hole of statistics. First let's observe that it is possible to take the expectation of some _function_ of a random variable. Suppose we have a random variable $X$, and at the resolution of a random process or experiment it takes a value. After it takes a value, we apply some function to that value. For example, we might have

$$
f(X) = 2X
$$
This function takes the result of the random process and multiplies it by 2. We shall calculate the expectation of this function of our random variable by 
...
::: {.example}
We will again use the distribution from example \@ref(exm:ogexpectation).
:::