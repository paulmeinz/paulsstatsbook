...
\end{align}
$$
Thus, $Var(X) = 1/4$. Congratulations on calculating your first variance!

:::

To further demonstrate the idea of variance as a measure of dispersion, let's do a small manipulation of the space of our random variable $X$ in example \@ref(exm:firstvariance).

::: .{example}
Let's multiply all the outcomes in the space of $X$ by 2 and call our new random variable $Z$ (in other words $Z = 2X$). Then $Z$ would have the pmf
...
\end{cases}
$$
I'll just go ahead and give you the fact that $E(Z) = 2$, but you should calculate this for yourself as well (you have all the tools, skill, and excitement to do it!). Next we'll calculate the variance

$$
\begin{align}
Var(Z) = \sum_{i=1}^{3} (z_1-\mu)^2p(z_i) &= (z_1-\mu)^2p(z_1) + (z_2-\mu)^2p(z_2) + (z_3-\mu)^2p(z_3) \\
