...
We shall refer to events in the space of $X$ using a simplified set notation. Suppose we are looking to determine the probability of a random variable $X$ attaining a value of 1 (more on this soon), then in the language of the last chapter we might write $\{x|X = 1\}$. Here, $\{x|X = 1\}$ describes the set of outcomes ($x$) in the space of $X$ where the random experiment results in 1. This is a small set composed of exactly one element - $\{1\}$. For the sake of simplicity, we will use the shorthand $(X = 1)$, or more generally $(X = x)$. We may also use an inequality - e.g., $X \leq x$ - to reference a range of outcomes. This shorthand is used frequently within the field of statistics and probability, so it's good to learn it now before we move into the more interesting stuff.  
...
Suppose we have a random variable $X$ with five outcomes in it's space - $\{1,2,3,4,5\}$. The pmf for $X$ assigns $\frac{1}{n}$ probability to each outcome. Here there are 5 outcomes, so
...
\end{align}
$$
We've multiplied each value in the space of our random variable $X$ by its probability and summed the values. The result of our computation is 3 - $E(x) = 3$ - but what does that mean exactly? Well let's explore the idea of expectation further by plotting the pmf of $X$. In the figure below, the probability of each outcome is plotted as a bar. You can see the outcomes of the random variable labelled on the $x$ axis. The probability of each outcome is represented by the bar height. In this case, each outcome is equilikely, so the bars are of equal height.  
...
```
Now suppose each of those bars has _mass_ (a.k.a. probability mass), and further suppose we were looking to balance that mass on a wedge or fulcrum. Just by looking at the plot, if we put a wedge directly under the value of 3, there would be equivalent probability mass on either side, and the plot would balance perfectly. The expectation can therefore be thought as the balancing point of the plot - the place that would perfectly balance the mass of probability for our random variable.
:::

Hopefully that example helped unpack the idea of the expectation. In this regard, we shall need a few more tools to proceed down the wonderful rabbit hole of statistics. First let's observe that it is possible to take the expectation of some _function_ of a random variable. Suppose we have a random variable $X$, and at the resolution of a random process or experiment it takes a value. We may apply some function to the outcomes in the space of $X$. For example, we might have
...
f(X) = 1
$$
Let's apply \@ref(eq:discreteexpectationfunction) to find the expectation of this function. First, let's apply the function $f(x)$ to each outcome in the space of $X$
...
$$
\begin{align}
E(X) = \sum_{i=1}^{5} f(x_i)p(x_i) &= 2p(x_1)+4p(x_2)+6p(x_3)+8p(x_4)+10p(x_5) \\
...
The astute observer might notice an interesting trend in the last two examples. In example \@ref(exm:exp1), we saw that our function which converts the whole space of $X$ into 1 had the expectation of 1. Indeed, looking back at the computation, we could perform that computation for any constant, e.g., if $c$ is a constant, and $f(X) = c$, then

$$
\begin{align}
E(X) = \sum_{i=1}^{5} f(x_i)p(x_i) &= \underline{c}\:p(x_1)+\underline{c}\:p(x_2)+\underline{c}\:p(x_3)+\underline{c}\:p(x_4)+\underline{c}\:p(x_5) \\
&= \underline{c}\:[p(x_1) + p(x_2) + p(x_3) + p(x_4) + p(x_5)] \\
&= \underline{c}\:[\frac{1}{5} + \frac{1}{5} + \frac{1}{5} + \frac{1}{5} + \frac{1}{5}] \\
&= \underline{c}(1) = c
\end{align}
$$

In other words, we would get c every time. Similarly, in \@ref(exm:exp2), recall that the original expectation of $X$ was 3. Our function $f(X) = 2X$ had the expectation of $2\times3 = 6$. That is, we multiplied $X$ by 2 and got back 2 times its expectation.

Our last two observations highlight a couple of _properties_ of the expectation. They are listed below. The first two you are familiar with (we just talked about them). The second two are new, and we'll pay particular attention to the fourth.

::: {.theorem #expprop name="Properties of the Expectation"}
If c is a constant, then

1. If f(X) = c, then E(f(X)) = E(c) = c

2. If f(X) = cX, then E(f(X)) = E(cX) = cE(X)

3. If f(X) = X + c, then E(f(X)) = E(X + c) = E(X) + c

4. If $X_1$ and $X_2$ are random variables, then E($X_1$ + $X_2$) = E($X_1$) + E($X_2$)
:::
