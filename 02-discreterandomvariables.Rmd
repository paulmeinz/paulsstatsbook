...
When we are in the process of conducting quantitative research (and statistics), we measure things in numbers. You may have noticed that our previous discussions of discrete sample spaces used non-numeric descriptions - e.g., $S = \{H, T\}$ in the case of a coin flip. To make these sample spaces easier to work with, we shall in practice map the outcomes in a discrete space to numbers. For example, if we were interested in a coin flip, we wouldn't work directly with the values of $H$ and $T$. We would map each outcome to a value, e.g., heads to the value of 1 and tails to the value of 0. On any given coin flip, the value of our outcome would therefore be a number. 

A variable that assumes a _number_ as a result of some random process (e.g., a flip, a random experiment, drawing out of a hat, rolling, randomly sampling, etc.) is called a **discrete random variable**. The set of possible numbers this variable can take is called the _space_ of the random variable, and like the discrete sample space, these outcomes shall be either finite or countable. In general we refer to a random variable with a capital letter, typically $X$. To avoid ambiguity, a potential _outcome_ of this random variable is referred to symbolically by a lower case letter, e.g., $x$. 
...
In practice, the set of rules (a.k.a, a probability mass function) are frequently described like this:
...
It is a measure of how the outcomes of $X$ are spread out about their mean (a.k.a. their _dispersion_). Some might wonder why we don't just calculate $E(X - \mu)$ as a measure of dispersion, and this shall motivate our first example
...
E(X) - \mu = \mu - \mu = 0
$$
The expectation of $X - \mu$ is zero which is not a very good measure of dispersion
:::
