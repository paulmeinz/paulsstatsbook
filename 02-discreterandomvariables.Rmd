...
It is a measure of the spread of the probability distribution for $X$ (a.k.a. the _dispersion_). There are indeed many ways you can measure the dispersion of a random variable. New students, for example, often wonder why we don't just calculate $E(X - \mu)$ as a measure of dispersion, and to answer this question, we'll do the calculation ourselves.

::: {.example}
Let's calculate $E(X-\mu)$ to understand why we can't use it as a measure of dispersion. This will also help us review properties of the expectation. First notice that $\mu$ is a constant, so
...
Now let's get back to work. Just like the expectation, we can also calculate the variance of some function of $X$. First calculate $E(f(X))$, referred to here as $\mu$, then 

$$
Var(X) = E((f(X)-\mu)^2) = \sum_{x}^{} (f(x) - \mu)^2p(x)
(\#eq:discretevariance)
$$
...
We shall observe one more thing about our last two examples. In the first example we found $Var(X) = 1/4$ and in the second example, we found $Var(2X) = 1$. Indeed, you may not have noticed this quickly (I didn't), but
...
\end{align}
$$
I might have lost you there. In words, it looks like the variance of $2X$ is simply 2 squared times the variance of $X$. This points to something that we observed for the expectation. Specifically, the variance also has properties for calculation. They make things way easier - especially given the computational intensity of finding the variance. 
...
3. More generally, $Var(aX + b) = a^2E(X) + 0$
