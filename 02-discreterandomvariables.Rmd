...
::: {.example}
Suppose we have three independent Bernoulli random variables - $X_1$, $X_2$, and $X_3$ - with with $p = 1/2$. We would like to know the probability that they sum to the value of 1. In order to get a value of 1 from our sum, we will need exactly 1 success and 2 non-successes at the resolution of our random process for each variable. There are three such combinations of outcomes that will yield this result. I've listed each in a set below. Note that the digit order in each outcome corresponds to variables 1, 2, and 3, respectively.
...
$$
P(\{1\,0\,0\})=(P(X_1 = 1)P(X_2 = 0)P(X_3 = 0) = 1/2 \times 1/2 \times 1/2 = 1/8 \\
P(\{0\,1\,0\})=(P(X_1 = 0)P(X_2 = 1)P(X_3 = 0) = 1/2 \times 1/2 \times 1/2 = 1/8 \\
P(\{0\,0\,1\})=(P(X_1 = 0)P(X_2 = 0)P(X_3 = 1) = 1/2 \times 1/2 \times 1/2 = 1/8
$$
Each of the outcomes above has the same number of successes and the same number of non-successes. Therefore each has the exact same probability, and by the law of addition, $P(Y) = 1/8 + 1/8 + 1/8 = 3/8$.
:::
...
First we will discuss the number of combinations, and to do this we will need a little bit of gentle review. Specifically, we will need to know the basics of the *factorial*. Given some integer $n$, a factorial is defined as
...
n! = 3\times2\times1 = 6
$$
We multiply $n$ by all the integers less than $n$ - all the way down to 1. Now you may recall (or be wondering) what we do with $0!$. A zero factorial is defined to be 1, so
