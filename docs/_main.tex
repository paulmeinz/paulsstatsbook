...
Before we get into it, we aren't going to learn set theory in it's entirety. That would be overly (and hilariously) ambitious. What we are going to learn will make understanding later sections easier, and it'll make it a bit easier to understand set notation in other circumstances. The first step in conducting a random experiment is clearly defining all the possible outcomes of your sampling procedure. This is referred to as the \textbf{sample space}. Suppose we have five different possible outcomes in our sample space: A, B, C, D, and E. Let's call this sample space \(S\). Then we would describe it like this:
...
The complement of a subset describes all the elements of a sample space \emph{not} in that subset. For the sake of consistency, we'll keep using our sample space \(S\), and let's work with the subset \$Y = \{\{A,B,C\} \$. The complement of that subset in the sample space would be \(\{D, E\}\). The operation of taking the complement of a subset is indicated by:
