...
As practitioners, we usually won't work directly with the probability density of a continuous random variable. Instead, we will work with something called a **cumulative distribution function** (cdf). Given a random variable with a probability density function, $f(x)$, a cdf determines the area under the density curve less than a given value of $x$. That is to say, it tells us the probability that the outcome of a random experiment falls below a value of $x$ - $P(X < x)$. We often refer to these functions with a capital $F$ to distinguish them from a probability density $f$. In other words,
