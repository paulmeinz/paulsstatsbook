...
Suppose we want to calculate $P(X < x)$ for several specified values of $x$ - .25, .5, .75, and 1. Call each of these values $x1$, $x2$, $x3$ and $x4$, respectively. We would like to calculate the area under the pdf from $-\infty$ up to each of these values. In the figure below, I've drawn a vertical line corresponding to each $x_i$, we shall need to calculate the area under $f(x)$ up to each one of those lines.
...
Thankfully, in this case, we won't need any fancy integration techniques to calculate a couple of values. We are again calculating the area of rectangles. This means that we simply need to multiply the length of the base of each of the rectangles (in this case $x_i$) above by its height (in this case, $1$). That is to say, we can proceed as in \@ref(exm:firstdensity). But before we do that, note that we are calculating the area under the pdf for *all* values less than $x$. For example, in the case of $x_1 = .25$, we will need to calculate the area under the curve from $-\infty$ all the way up to 1. That's not a rectangle (oh, the horror!). Thankfully, our pdf says that any value less than 0 has a probability density of 0, so we can safely assume that the area under the curve for any value less than zero is zero. 
