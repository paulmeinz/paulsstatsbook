...
The calculation of a continuous expectation is a bit more complex, and requires integration techniques which are beyond the scope of this book. However, the intuition for the calculation is the same. We'll walk through it right now. Consider the probability density function
...
\end{cases}
$$
Recall, that with regards to the discrete case, the expectation is calculated by multiplying each value of $X$ by its corresponding probability $p(x)$ and summing the results. That is, if there are $n$ outcomes in the space of $X$,

$$
E(X) = \sum_{i = 1}^{n}x_ip(x_i)
$$
Let's try something similar, first we'll divide the support of our random variable $X$ into four equal ranges - each with width $\frac{1}{4}\sqrt{2}$. Specifically,
...
With more ranges - and smaller range widths - the value of our approximation gets closer and closer to the actual value. This observation leads us to an intuitive interpretation of how $E(X)$ is calculated for a continuous random variable. Suppose we were to make the ranges extremely small (and the number of ranges very high) and perform the computation above. That is, we take the probability that $X$ falls in each of our very small range, multiply it by a value of $x$ in that extremely small range, and then sum the resulting values. We would end up with the value of $E(X)$.
