...
Now, let's plot each of our values of $x$ and it's corresponding $P(X < x)$. Ultimately this will help us figure out the cdf of $f(x)$. We have four $(x,y)$ coordinates - $(.25, P(X < .25))$, $(.50, P(X < .50))$,$(.75, P(x < .75))$, and $(1, P(X < 1))$. If we plot each and connect each with a line, we get
...
::: {.example #exp}
Suppose we are working with our previously described exponentially distributed random variable, and we would like to know the probability that the outcome of a random experiment falls between 2 and 3. That is, we would like to know $P(2 < X < 3)$. Using our cdf, we could easily calculate $P(X < 3)$ but that would include all the probability mass in the region below 2. That's not what we want. Thankfully, we can still work with our cdf. In the solution that follows below, we shall approach the problem in two ways - one using set notation and another that leans on intuition and graphs.

Take a close look at the range of $X < 3$. It can actually be written as the union of two mutually exclusive events, e.g.,

$$
\{X < 3\} = \{X < 2\} \cup \{2 \leq X < 3\}
$$
We have two mutually exclusive events which means we can use the law of addition \@ref(eq:addlaw) to get what we need! To see how, notice that (because the law of addition)

$$
P(\{X < 2\} \cup \{2 \leq X < 3\}) = P(X < 3) \\
P(X < 2) + P(2 \leq X < 3) = P(X < 3)
$$
Note I dropped the curly braces in the second line for the sake of simplicity. We can now subtract $P(X < 2)$ from both sides and voila
...
\end{align}
$$
From the above, we can conclude that given some $a$ and $b$, we can calculate $P(a < X < b)$ by

$$
P(a < X < b) = P(X < b) - P(X < a) = F(b) - F(a)
(\#eq:fundamental)
$$
Note that a and b were 2 and 3 in our example, respectively. An astute student (and I believe you are an astute student) may notice that in our example we calculated $P(2 \leq X < 3)$, and we have just provided an equation for $P(2 < X < 3)$. However, recall our brief discussion in the section before this - $P(a \leq X < b) = P(a < X < b) = P(a < X \leq b) = P(a \leq X \leq b)$. If you continue your studies in mathematics, the equation above isn't just special to probability and statistics. In fact, it's one part of a very important theorem that is so important it has a fancy name - the Fundamental Theorem of Calculus.

Now that we've worked with sets, we shall turn our attention to an intuitive approach. The gray colored region in the plot below is the area under the curve left of the value 3 (e.g., $x < 3$). As we have learned, it represents $P(X < 3)$. The area between the two vertical lines at the values 2 and 3 under the curve represents $P(2 < X < 3)$, and the area to the left of 2 represents $P(X < 2)$. Looking at the two regions, one can see that $P(X < 2)$ and $P(2 < X < 3)$ combine (sum) to $P(X < 3)$. We shall need to subtract out the area left of 2 to get our desired value.
...
Note that the support of $X$ is $X \geq 0$, and therefore, $P(X \geq 0) = 1$. That is to say, a random experiment would have a 100% chance of yielding a value greater than or equal to 0. Also, observe that

$$
\{0 < X\} = \{0 \leq X \leq 3\} \cup \{3 < X\} 
$$
In English: {0 < X} is the union of two mutually exclusive sets - $0 \leq X \leq 3$ and $3 < X}$. We can once again use the law of addition to get our desired probability.

$$
\begin{align}
\{0 < X\} &= \{0 \leq X \leq 3\} \cup \{3 < X\} \\
P(\{0 < X\}) &= P(\{0 \leq X \leq 3\} \cup \{3 < X\}) \\
1 &= P(0 \leq X \leq 3) + P(3 < X) \\
1 - P(0 \leq X \leq 3) &= P(3 < X)
\end{align}
$$
This means that all we need is $P(0 \leq X \leq 3)$ and we will be able to calculate our desired probability. Looking at the pdf of $X$, one can see that there is no area below the curve for $x$ less than zero (that is, $f(x) = 0$). In other words, $P(0 \leq X \leq 3)$ is equal to $P(X < 3)$, and we know the latter because of our cdf. Specifically 
