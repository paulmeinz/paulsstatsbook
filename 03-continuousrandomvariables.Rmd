...
The interpretation of independence in the context of continuous random variables is (again!) quite similar to that of discrete random variables. A thorough look into the dependence/independence of continuous random variables would require a little too much integration for our needs. Instead, we'll focus on independence. Later in this book, we'll discuss dependence a little bit more in the context of correlation and normally distributed random variables (we'll learn about normal distributions in the next section).

Suppose we are given two **independent** random variables $X$ and $Y$ with cdfs $F_X(x)$ and $F_Y(y)$, respectively. I've used the subscripts on each cdf to make it extra clear to which random variable they belong. Further suppose we are given two events $A$ and $B$ in the space of $X$ and $Y$, respectively, and we would like to know $P(A \cap B)$. Then, just as with discrete random variables    
