...
Now, suppose we conduct a random experiment with our previously described sample space $S = \{A, B, C, D, E\}$. The _outcome_ of our experiment is the single item that results from our *random process*, e.g., a random draw with eyes closed, a dart board with a blind fold, etc. In practice, we will frequently talk about outcomes in terms of _events_. Events are described by a *subset* of distinct elements from the sample space. As opposed to an outcome, they may be one or more elements of the sample space. For example, we might describe an event as $C = \{A, B, C\}$, and if our outcome (a single item) lands in the that subset (it is either A, B, or C), we shall say the event "occurred". For the sake of brevity, and positivity, I will frequently refer to the occurrence of an event as a "success" (congratulations all around!). 
...
Indeed, in practice we might want to get a little more creative in terms of the outcomes and events associated with our random experiment. For example, we might be interested in a combination of and/or in the shared elements of multiple events. We shall therefore need a language to describe more complicated scenarios. The next few examples will describe (at a basic level) how we might "combine" events in different ways. 

::: {.example}

Suppose we have two events $C_1 = \{A,B,C\}$ and $C_2 = \{B,C,D\}$, and we are interested in whether or not our outcome lands in $C_1$ *or* $C_2$. Let us first consider the circumstances where our experiment would result in a success. We might reason that if the outcome of our random process was an A, B, C, or D, then our outcome will have been a success. Specifically, if our outcome were an A, then it has landed in $C_1$; a B or C, then it has landed in $C_1$ and $C_2$; or a D, then it has landed in $C_2$. Any other outcome would result in an non-success (or dare I say, a "failure"). Indeed, you'll note that our outcome must be one of the *unique* elements in either set otherwise we're toast. 
...
Here's another example, suppose we are interested in whether or not our outcome falls into $C_1 = \{A\}$ or $C_2 = \{B\}$. A success would occur if our outcome were either an $A$ or a $B$. Using our newfangled union operator, we can write the union of these two sets as
...
We are therefore interested in the set of _shared_ elements of our two events. If our outcome lands in this set of share elements, then we are sure to be successful. The *intersect operator* ($\cap$) will help us signify this set:
...
Well, you may have guessed it: Our two sets had no overlap. You couldn't possibly draw a single element that landed in _both_, because the sets share no elements. As a result, we get a very special set - a *null set* - $\{\}$ - which is a set with zero elements. In this circumstance (a.k.a. they have no overlap), we shall refer to the two sets as **mutually exclusive** - because they share no elements. In terms of probability, mutually exclusive sets can be much easier to work with. We'll talk about this soon.
:::

The union and intersect operators, along with mutual exclusiveness, shall be very important workhorses for describing complicated events. In the next section, you will see that they have an important roll in how we characterize the randomness of our random experiment. At this point, I would feel remiss if I didn't dive into the null set a bit more. It shall also have an important roll in how we calculate randomness, so let's take a look.
...
::: {.example}
Let's try one more example with $C_1$ and $C_2$ from the last example. Let's say we defined a successful outcome as one that does not simultaneously fall in $C_1$ AND $C_2$. Notice the important "AND". Our list of unsuccessful items will be different this time because we are referring to the shared (intersect) of both our subsets. Therefore:
