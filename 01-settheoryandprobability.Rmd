...
Wow, that was an interesting result! Our two sets had no overlap. You couldn't possibly draw a single element that landed in _both_ the sets. The are two lessons to learn from this. First, we get a very special set - a *null set* - $\{\}$ - which is a set with zero elements. Second, when the intersect of sets results in a null set (a.k.a. they have no overlap), then the sets are called **mutually exclusive**.

Regarding the former point, it's fun and interesting to see how a null set works with our operators. For example:
...
With a good understanding of some of the basic operators and sets, let's turn to a brief treatment of the continuous sample space. The continuous case is very similar to the discrete case - although it takes some drawing of number lines if you are rusty with inequalities (no problem). Suppose we are sampling from a sample space $S = \{x\,|\,0 \leq x \leq 10 \}$ and we are interested in if the event lands in $C_1 = \{x\,|\,0 \leq x \leq 2 \}$ OR $C_2 = \{x\,|\,2 \leq x \leq 5 \}$. A successful sample here would occur if our realized element landed anywhere from zero to five. In other words:
...
The introduction of sets was dedicated to a random experiment where we only draw a single element from our sample space. In practice, researchers and statisticians are often sampling from the same sample space (or multiple equivalent sample spaces), repeatedly. Here we'll describe the sample spaces which include multiple draws. Note, for the purposes of our learning, we shall only be discussing sampling with _replacement_. That is, we will be talking about circumstances where the 1st, 2nd, 3rd, ... , nth sample item gets returned to the sample space each time. This is opposed to the circumstance where a draw/realization reduces the size of the sample space each time.

To understand how to describe the sample spaces mentioned above, we'll turn to a good-old-fashioned coin toss (the mainstay of every introductory probability book). Suppose we perform a single flip (draw) from a fair coin. Our sample space would be $S = \{H, T\}$ where $H$ is "heads", and $T$ is "tails". Now let's suppose we conduct two flips. The sample space would be all the possible combinations of heads and tails from the two flips, and an event is some subset of this space. It helps to draw draw a _tree diagram_ to get the gist of it.