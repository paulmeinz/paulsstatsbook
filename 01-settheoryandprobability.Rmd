...
Suppose we have two events $C_1 = \{A,B,C\}$ and $C_2 = \{B,C,D\}$, and we are interested in whether or not our outcome lands in $C_1$ *or* $C_2$. Let us first consider the circumstances where our experiment would result in a success. We might reason that if the outcome of our random process was an A, B, C, or D, then our outcome will have been a success. Specifically, if our outcome were an A, then it has landed in $C_1$; a B or C, then it has landed in $C_1$ and $C_2$; a D, then it has landed in $C_2$. Any other outcome would result in an non-success (or dare I say, a "failure"). Indeed, you'll note that our outcome must be one of the *unique* elements in either set otherwise we're toast. 
...
Suppose we have our two previously described events - $C_1 = \{A,B,C\}$ and $C_2 = \{B,C,D\}$ - each a subset of $S$. Let's further say that we are interested in whether or not both events occur as a result of our random experiment. Once again, let's think about the circumstance where this might occur. We shall be successful if our outcome falls in both $C_1$ *and* $C_2$. In this case, an outcome of $A$ or $D$ would absolutely not suffice. $A$ is an element of $C_1$ alone and $B$ is an element of $C_2$ alone. Our outcome better be $B$ or a $C$. 

We are therefore interested in the set of _shared_ elements of our two events. The *intersect operator* ($\cap$) will help us signify this set of share elements:

$$ C_1 \cap C_2 = \{A,B,C\} \cap \{B,C,D\} = \{B,C\} 
(\#eq:intersect)$$
:::

In this context, unlike $\cup$, the intersect operator represents **and**. It tells us the conditions for success if we would like _both_ events to occur (a.k.a., our outcome landing in both events). This operator can create some interesting scenarios, so it's worthwhile to do another example. 
...
Well, you may have guessed it: Our two sets had no overlap. You couldn't possibly draw a single element that landed in _both_, because the sets share no elements. As a result, we get a very special set - a *null set* - $\{\}$ - which is a set with zero elements. In this circumstance, we shall refer to the two sets as **mutually exclusive** - because they share no elements. In terms of probability, mutually exclusive sets can be much easier to work with. We'll talk about this soon.
:::

The union and intersect operators, along with mutual exclusiveness, shall be very important workhorses for describing complicated events. In the next section, you will see that they have an important roll in how we characterize the randomness of our random experiment. We'll turn to that conversation soon. If I'm being honest, the next section is far more interesting than this one. However, I would feel remiss if I didn't dive into the null set a bit more. It shall also have an important roll in how we calculate randomness, so let's take a look.

::: {.example}
In particular, it is interesting to contemplate how the null set "interacts" with other sets/events. For example:

$$ \{A,B,C,D,E\} \cap \{\} = \{\} $$ 

A set with elements intersected with a null set is a null set. A set with something in it can share nothing with a set that has no elements at all. This is very similar to multiplying an number by zero. In fact, when you first start writing set notation, you might be inclined to use arithmetic operators with sets, e.g., $\{A,B,C\} - \{A,B\} = \{C\}$ as opposed to $\{A,B,C\} \cap \{A,B\} = \{C\}$. The null set and intersect gives us a handy way to multiply a set by zero without using $\times$ or 0. 
...
Now that we have our subset of unsuccessful elements, we'll just take the complement of that:

$$ X^c = \{E\} $$

Of course, you might have known the answer to this all along, but it's nice to see the steps broken down exactly. We could have written it all in one go using parentheses to specify order of operations, e.g.:
...
::: {.example}
Let's try one more example with $C_1$ and $C_2$ from the last example. Let's say we defined a successful outcome as one that does not simultaneously land in $C_1$ AND $C_2$. Notice the important "AND". Our list of unsuccessful items will be different this time because we are referring to the shared (intersect) of both our subsets. Put another way, our experiment would be a non-success if the outcome were an element of both $C_1$ and $C_2$ Therefore:
