...
Indeed, in practice we might want to get a little more creative in terms of the outcomes and events associated with our random experiment. For example, we might be interested in a combination of and/or the shared elements of events. We shall therefore need a language to describe more complicated scenarios. The next few examples will describe (at a basic level) how we might "combine" events in different ways. 
...
Hopefully the last two examples gave you a serviceable understanding of the union operator and its function. It is nevertheless important to reiterate that we would use the union operator when we are interested in the occurrence of one event *or* another. In this circumstance, we would be perfectly satisfied with either event occurring. BUT, what if we were interested in whether or not *both* events occurred, that is, whether or not our outcome landed in both $C_1$ *and* $C_2$. We'll talk about that scenario next.
