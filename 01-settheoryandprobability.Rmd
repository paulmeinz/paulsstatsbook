...
In the course of research, behavioral scientists will often take samples of human behavior, e.g., a survey of a group of people, measurements of reaction time, etc. These samples will take some _outcome_, and most of us understand and have observed the inherent randomness of this act. That is to say, you might take multiple samples in a carefully controlled circumstance and get different results every time.

Statisticians often refer to an analogous circumstance when talking about probability. Suppose we are able to describe all the possible outcomes of our sampling procedure. We take samples over and over again in the same circumstance, and each time the outcome may change. This process is commonly referred to as a **random experiment**. In order to familiarize you with some introductory concepts around probability, we're going to start within the context of a random experiment. First, we'll learn about sets and set theory in order to describe a **sample space** - or the set of outcomes our random experiment may take. Then we'll more concretely define the randomness of our experiment by talking about **probability** - the quantification of randomness.

## Sets and Sample Spaces

In our quest to define sample spaces, the first thing we shall need to learn about are things that statisticians call **sets** - collections or lists of elements. We aren't going to embark on an in depth foray into set theory. That would be overly (and hilariously) ambitious, and I would be horribly unprepared to teach you. What we are going to learn will make understanding later sections easier, get your brain warmed up, and make it a bit easier to understand set notation in other circumstances. 

The first step in conducting a random experiment is clearly defining all the possible outcomes of your sampling procedure. As previously mentioned, this *set* of potential outcomes is referred to as the **sample space**. For example, suppose we have five different possible outcomes (a.k.a., elements) in our sample space - A, B, C, D, and E. Calling our set $S$ ($S$ for Sample!), and using *set notation*, we would describe our sample space like this

$$
S = \{A, B, C, D, E\}
(\#eq:discreteset)
$$
That is to say we can describe $S$ with curly braces surrounding a list of _distinct_ elements in the set. The set $S$ is an example of a **discrete** sample space - meaning it either contains a finite number of elements or it is _countable_. In this case, the sample space has a finite number of elements (five to be exact), so we can call it discrete. 
...
The inclusion criteria for the set is written after the $|$ in the curly braces. The $|$ can be read loosely as meaning "where", so reading the whole thing in the curly braces left to right you get x _where_ $0 \leq x \leq 1$. Therefore, our set is all the $x$'s from 0 to 1. Note that I will (and you can) drop the bit before and including the $|$ when it is clear what variable is referenced. As an aside, continuous sample spaces can be found everywhere in the field of behavioral statistics. For reasons that will become apparent later, we shall nonetheless approach these spaces with slightly less mathematical depth. 

Now, suppose we conduct a random experiment with our previously described sample space $S = \{A, B, C, D, E\}$. The _outcome_ of our experiment is the single item that results from our *random process*, e.g., a random draw with eyes closed, a dart board with a blind fold, etc. In practice, we will frequently talk about outcomes in terms of _events_. Events are described by a *subset* of distinct elements (or potential outcomes) from the sample space. They may be described by *any number of elements* from the sample space. For example, we might describe an event as $C = \{A, B, C\}$, and if our outcome (a single item) lands in the that subset (it is either A, B, or C), we shall say the event "occurred". For the sake of brevity, and positivity, I will frequently refer to the occurrence of an event as a "success" (congratulations all around!). 

You can indicate a set is a subset by:
...
Indeed, in practice we might want to get a little more creative in terms of the potential outcomes and events associated with our random experiment. For example, we might be interested in a combination of and/or in the shared elements of multiple events. We shall therefore need a language to describe more complicated scenarios. The next few examples will describe (at a basic level) how we might "combine" events in different ways. 

::: {.example}

Suppose we have two events $C_1 = \{A,B,C\}$ and $C_2 = \{B,C,D\}$, and we are interested in whether or not our outcome lands in $C_1$ *or* $C_2$. Let us first consider the circumstances where our experiment would result in a success. We might reason that if the outcome of our random process was an A, B, C, or D, then our outcome will have been a success. Specifically, if our outcome were an A, then it has landed in $C_1$; if it were an a B or C, then it has landed in $C_1$ and $C_2$; and if it were a D, then it has landed in $C_2$. Any other outcome would result in an non-success (or dare I say, a "failure"). Indeed, you'll note that our outcome must be one of the *unique* elements in either set. Otherwise we're cooked. 

Following our logic, we could take the unique outcomes of either set and combine them into a new set. If our outcome lands in that new subset, we can be confident that $C_1$ or $C_2$ occurred. There's a handy symbol that indicates this operation:

$$ C_1 \cup C_2 = \{A,B,C\}\:\cup\:\{B,C,D\} = \{A,B,C,D\} 
(\#eq:union) $$
:::

This **union operator** ($\cup$) takes the unique elements of both sets and puts them into a set of their own. In this circumstance it is used synonymously with *or*. If the event of interest is our outcome landing in $C_1$ OR $C_2$, then we would be successful if the outcome were a unique element from both sets. 

::: {.example}

Here's another example, suppose we are interested in whether or not our outcome falls into $C_1 = \{A\}$ or $C_2 = \{B\}$. Intuition tells us that a success would occur if our outcome were either an $A$ or a $B$. Using our newfangled union operator, we can write the union of these two sets as
...
Hopefully the last two examples gave you a serviceable understanding of the union operator and its function. To reiterate: we would use the union operator when we are interested in the occurrence of one event *or* another. In the "or" circumstance, we would be perfectly satisfied with our outcome landing in any single event (e.g., $C_1$). However, what if we were interested in observing the occurrence of *both* events, that is, whether or not our outcome landed in both $C_1$ *and* $C_2$? We'll talk about that scenario next.

::: {.example}

Suppose we have our two previously described events - $C_1 = \{A,B,C\}$ and $C_2 = \{B,C,D\}$ - each a subset of $S$. Let's further say that we are interested in whether or not both events occur as a result of our random experiment. Once again, let's think about the circumstance where this might occur. We'll first observe that an outcome of $A$ or $D$ would absolutely not suffice. $A$ is an element of $C_1$ alone and $B$ is an element of $C_2$ alone. Either of those outcomes would result in the occurrence of just one outcome. Indeed, comparing the two sets, one might note that $B$ and $C$ are present in both. If we are hoping for the occurrence of $C_1$ and $C_2$, our outcome better be one of those two.

Generally speaking, in the case of the "and" scenario, we are interested in the set of _shared_ elements of our two events. The *intersect operator* ($\cap$) will help us signify this set of share elements. In terms of our example, then,

$$ C_1 \cap C_2 = \{A,B,C\} \cap \{B,C,D\} = \{B,C\} 
(\#eq:intersect)$$
:::

Unlike $\cup$, the intersect operator represents *and*. It tells us the conditions for success if we would like _both_ events to occur (a.k.a., our outcome landing in both events). This operator can create some interesting scenarios, so it's worthwhile to do another example. 

::: {.example}

This time let's define our events to be $C_1 = \{A,B\}$ and $C_2 = \{C,D\}$. Now we're interested in whether or not our outcome falls in $C_1$ and $C_2$ (What do you think is going to happen here?). Using the intersect operator

$$ C_1 \cap C_2 = \{A, B\} \cap \{C, D\} = \{\} $$

Well, you may have guessed it: Our two sets had no share elements. You couldn't possibly draw a single element that landed in _both_, because the sets share no elements. As a result, we get a very special set - a *null set* - $\{\}$ - which is a set with zero elements. When the intersect of two sets is the null set, we shall refer to the two sets as **mutually exclusive** - because they share no elements. In terms of probability, mutually exclusive sets can be much easier to work with. We'll talk about this soon.
:::

The union and intersect operators, along with mutual exclusiveness, shall be very important workhorses for describing complicated events. In the next section, you will see that they have an important roll in how we characterize the randomness of our random experiment. We'll turn to that conversation soon (get excited!). I would nevertheless feel remiss if I didn't dive into the null set a bit more. It will also have an important roll in how we calculate randomness. Let's take a deeper look at some of its properties.

::: {.example}
It is interesting to contemplate how the null set "interacts" with other sets/events. For example:

$$ \{A,B,C,D,E\} \cap \{\} = \{\} $$ 

In words, when we intersect a non-empty set with a null set, we get a null set. Intuitively speaking, this makes sense. A set with something in it can't possibly share elements with a set that has no elements at all. This is very similar to multiplying an number by zero. In fact, when you first start writing set notation, you might be inclined to use arithmetic operators with sets, e.g., $\{A,B,C\} - \{A,B\} = \{C\}$ as opposed to $\{A,B,C\} \cap \{A,B\} = \{C\}$. The null set and intersect gives us a handy way to multiply a set by zero without using $\times$ or 0. 

Given the above conversation, what do you suppose happens when we union a null set with something else? Let's try it out:

$$ \{A,B,C,D,E\} \cup \{\} = \{A,B,C,D,E\} $$

A set unioned with a null set is itself. A null set has no elements to add to our "something set". Although the null set is really cool (in my opinion; and I hope your opinion too), working with it is, metaphorically speaking, like talking to the most boring person in the world. You have nothing in common with them ($\cap$), and they have nothing to add ($\cup$). And, with that, you have learned your first set theory insult.
:::

Up to this point, we've learned about sample spaces, the concept of an outcome and event, the union/intersect (or/and) operators, and the fancy null set. That's about all we need. I would like to finish out this brief section on sets with a discussion of the **complement** of a subset and a brief treatment of how the previously mentioned operators work in the continuous case.

The complement of a subset describes all the elements of a sample space _not_ in that subset. For the sake of consistency, we'll keep using our sample space $S$ \@ref(eq:discreteset), and let's work with the event $Y = \{A,B,C\}$, $Y\subset S$. The complement of that subset in the sample space would be $\{D, E\}$. The operation of taking the complement of a subset is indicated by:

$$ Y^c = \{A,B,C\}^c = \{D,E\} 
(\#eq:complement)$$

As you will see, the complement of a set allows us to signify a wider breadth of operations. 

::: {.example}
Let's again consider our sample space $S$. Suppose we were interested in an event composed of the elements not in $C_1 = \{A,B\}$ or $C_2 =\{C,D\}$. It is helpful here to consider all the outcomes of our random experiment that would result in a non-success. Specifically, we'd be out of luck if our outcome were an element of $C_1$ or $C_2$. Recall that the $\cup$ (union) gives us the unique elements in both sets. Let's take these two sets and union them into a set $X$ together.

$$ C_1 \cup C_2 = \{A,B,C,D\} = X $$

The elements of $X$ are the potential outcomes for which we would experience failure (sadness). Now if we just take the complement of that, we'll get the event we're interested in

$$ X^c = \{E\} $$

An $E$ would therefore result in a success. Of course, you might have known the answer to this all along, but it's nice to see the steps broken down exactly. We could have written it all in one go using parentheses to specify order of operations, e.g.:
...
::: {.example}
Let's try one more example with $C_1$ and $C_2$ from the last example. Let's say we defined a successful outcome as one that does not land in $C_1$ AND $C_2$. Notice the important "AND". Our list of unsuccessful items will be different this time because we are referring to the shared (intersect) of both our subsets. Put another way, our experiment would be a non-success if the outcome were an element of both $C_1$ and $C_2$ Therefore:

$$ (C_1 \cap C_2)^c = \{\}^c = \{A,B,C,D,E\} $$

The two sets have no overlap, so their intersect is a null set, and the complement of the null set is all the elements in our sample space. As it turns out, in this case, we can't be unsuccessful.
:::

With a good understanding of some of the basic operators and sets, let's turn to a brief treatment of the continuous sample space. The continuous case is very similar to the discrete case - although it takes some drawing of number lines if you are rusty with inequalities (no problem; we all get rusty sometimes). 

::: {.example}
Suppose we are conducting a random experiment with a sample space $S = \{x\,|\,0 \leq x \leq 10 \}$. Further suppose that we are interested in the occurrence of at least one of two events - $C_1 = \{x\,|\,0 \leq x \leq 2 \}$ OR $C_2 = \{x\,|\,2 \leq x \leq 5 \}$. A successful result here would occur if our outcome landed from 0 to 2 or from 2 to 5. In other words:

$$ \{0 \leq x \leq 2 \}\:\cup\:\{2 \leq x \leq 5\} = \{0 \leq x \leq 5\} $$

$C_1$ or $C_2$ implies a success in the range from (and including) 0 to 5. Now, what if we wanted our outcome to land in both sets? Let's keep the same continuous sample space and subsets as above, and this time look at the intersect of the two sets:

$$ \{0 \leq x \leq 2 \}\:\cap\:\{2 \leq x \leq 5\} = \{2\} $$

The two sets only overlap at exactly $x = 2$. If we want $C_1$ and $C_2$ to occur, we'll need to hope for a value of 2 exactly. Otherwise, we're out of luck. Let's do a couple more so you can get a sense for it. 
:::

::: {.example}
Suppose we have a sample space $Q = \{x\,|\, 5 \leq x \leq 10\}$ and define $C \subset Q$ where $C = \{x\,|\, 5 \leq x < 7\}$. That is to say, $C$ is a subset of Q, and its interval includes the lower side (5) but it doesn't include the upper side (7). What is the complement of $C$? Using what we've learned with discrete spaces
...
```{r fig1, echo = FALSE, fig.cap = "Plotting a Complement"}
x <- c(5,7)
y <- c(.3,.3)
C <- data.frame(x,y)
...
I will end this section with an important discussion on notation. As you can see from \@ref(eq:permutations), the sample space can grow quite rapidly. For example, four coin flips would have a sample space of 16 elements. Consider the two coin sample space - $\{HH,HT,TH,TT\}$. The event of getting a head on the first flip can be described by all the coin flip combinations with heads on the first flip - $\{HH,HT\}$. If our random experiment and outcome fell into this subset, we would indeed have flipped a heads on the first coin. Now consider a circumstance where the sample space is much larger (e.g., 10 coins), it would become quickly untenable to write out all the possible permutations. Instead in some circumstances I may use some short hand like or similar to $\{H \: on \: 1st \: flip\}$, because I don't have the rest of my natural born life to list permutations (and you don't have the rest of your natural born life to read them). Rest assured that these are exactly equivalent, and they shall be associated with the exact same probabilities. Indeed, sometimes the short hand will make it more clear as to which probability we are referring to.

### Practice Problems
Since this is the first set of practice problems in the book. I feel it's important to make a point: Being incorrect is an extraordinary important part of learning. Rest assured that when I present you with a difficult problem, I am not intending to highlight your deficits. I am hoping that you are challenges to learn things on a slightly deeper level.

For questions 1 through 7 below, consider the sample space $S = \{A,B,C,D,E,F,G,H,I,J\}$ and the two events $C_1 = \{A,E,F,G\}$ and $C_2 = \{A,F,I,J\}$.

1. What is $C_1 \cup C_2$

2. What is $C_1 \cap C_2$

3. What is $C_1^c$

4. Using $C_1$ and $C_2$ and the set operations we've learned (the union, intersect, and complement), how might we specify the elements not in $C_1$ or $C_2$?

5. Let $C_3 = {J}$. What elements are in the set $(C_1 \cup C_2)^c \cap C_3$

6. Let $C_3 = {J}$. What elements are in the set $(C_1 \cup C_2)^c \cup C_3$

7. Suppose we have two sets $A$ and $B$, and $A \cup B = A$, what elements could B contain?

8. Suppose we have two sets $A$ and $B$, and $A \cap B = A$, what elements could B contain?

9. Suppose we have a sample space $S = \{A,B,C,D,E\}$ and two events $A = \{A,B\}$ and $B = \{C, D\}$. What is $A^c \cup B^c$? What is $(A \cap B)^c$?

10. Suppose we have a sample space $S = \{A,B,C,D,E\}$ and two events $A = \{A,B,C\}$ and $B = \{C,D\}$. What is $A^c \cap B^c$ What is $(A \cup B)^c$?

For questions 10 through 15 below, consider the sample space $S = \{x| 0 \leq x \leq 10\}$ and the two events $C_1 = \{0 \leq x < 2\}$ and $C_2 = \{2 \leq x < 5\}$

11. What is $C_1 \cap C_2$?

12. What is $C_1 \cup C_2$?

13. What is $C_1^c$?

14. What is $(C_1 \cap C_2)^c$

15. Provide an example of a set $A$ for which $C_1 \cup A = C_1$.
16. Suppose three standard six-sided dice are rolled. What is the size of the sample space (how many elements) of this experiment?
17. Suppose a coin is flipped, a six-sided dice is rolled, and a card is drawn at random from a fifty-two card deck. What is the size of this sample space?

### Practice Problem Answers
1. $\{A,E,F,G,I,J\}$
2. $\{A,F\}$
3. $\{B,C,D,H,I,J\}$
4. Take the complement of $C_1 \cup C_2$, or using parentheses to specify order of operations - $(C_1 \cup C_2)^c$
5. $\{J\}$
6. $\{A,E,F,G,I,J\}$
7. $B$ could contain no elements (the null set) or any subset of elements in $A$.
8. $B$ would need to equal $A$.
9. Both are the same - $\{A,B,C,D,E\}$. More generally, $A^c \cup B^c = (A \cap B)^c$
10. Both equal $\{E\}$. More generally, $A^c \cap B^c = (A \cup B)^c$
11. $\{2\}$
12. $\{0 \leq x < 5\}$
13. $\{2 \leq x \leq 10\}$
14. $\{0 \leq x < 2\} \cup \{2 < x \leq 10\}$ - everything but 2.
15. $\{1\}$ would work.
16. $6^3 = 216$
17. 624
...
```{r, echo = FALSE, fig.cap = "Probabilities before C2 occured"}
x <- c(1/3,1/3,1/3)
y <- as.factor(c(1,2,3))
C <- data.frame(x,y)
...
```{r, echo = FALSE, fig.cap = "Conditional probability now that C2 has occurred"}
x <- c(1/3,1/3,1/3)
y <- as.factor(c(1,2,3))
C <- data.frame(x,y)
